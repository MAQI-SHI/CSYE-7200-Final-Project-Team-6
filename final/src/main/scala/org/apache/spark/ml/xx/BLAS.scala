package org.apache.spark.ml.xx
...
object BLAS extends Serializable with Logging {
  ...
  @transient var _f2jBLAS: NetlibBLAS = _
  @transient var _nativeBLAS: NetlibBLAS = _
  ...
  def syr(alpha: Double, x: DenseVector, A: DenseMatrix): Unit = {
    val nA = A.numRows
    val mA = A.numCols
  ...
  def syr(alpha: Double, x: SparseVector, A: DenseMatrix): Unit = {
    val mA = A.numCols
    val xIndices = x.indices
    val xValues = x.values
